...
@njit(cache=True)
def process_and_draw_triangles(triangles_array: np.ndarray,
    ...
    for triangle_idx in range(len(triangles_array)):
        ...
        if not fully_visible:
            ...
            # Allocate buffers for clipping
            max_no_clipped_triangles = 8

            # Process each frustum plane
            current_triangles = np.zeros((max_no_clipped_triangles, 3, 3), dtype=np.float32) 
            current_triangles[0] = vertices
            
            if shading_mode == 'gouraud' and has_vertex_normals:
                current_normals = np.zeros((max_no_clipped_triangles, 3, 3), dtype=np.float32)
                ...
                for i in range(len(frustum_planes)):
                    ...
                    next_triangles = np.zeros((max_no_clipped_triangles, 3, 3), dtype=np.float32) 
                    next_normals = np.zeros((max_no_clipped_triangles, 3, 3), dtype=np.float32)
                ...
                for i in range(num_triangles):
                    project_and_draw_triangle(
                        current_triangles[i], vertex_normals, 
                        shading_mode,
                        canvas_width, canvas_height,
                        viewport_width, viewport_height,
                        canvas_buffer, depth_buffer,
