...
@njit(cache=True)
def project_vertex(vertex: np.ndarray,
                  canvas_width: int, canvas_height: int,
                  viewport_width: float, viewport_height: float) -> Tuple[float, float, float]:
    """
...
@njit(cache=True)
def transform_vertices_and_normals(vertices: np.ndarray, vertex_normals: np.ndarray, translation: np.ndarray, rotation: np.ndarray, scale: np.ndarray,
                                    camera_translation: np.ndarray, camera_rotation: np.ndarray, shading_mode: str, has_vertex_normals: bool) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    for i in range(num_vertices):
        # Transform vertex
        transformed_vertices[i] = transform_vertex(vertices[i], translation, rotation, scale,
        ...
        if shading_mode == 'gouraud' and has_vertex_normals:
            transformed_normals[i] = transform_vertex_normal(vertex_normals[i], -rotation, camera_rotation, True)
