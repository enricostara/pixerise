    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # Left click
                ...
                if selected_cube:
                    scene._instances[selected_cube].set_color(255, 255, 255)

                # Update new selection
                if hit:
                    instance_name, _ = hit
                    print(f"Selected cube: {instance_name}")
                    scene._instances[instance_name].set_color(255, 0, 0)
...
def main():
    ...
    viewport = ViewPort(
        (1.6, 1.2), 1, canvas
    )  # Adjusted viewport size for better perspective
    ...
    # Set up camera
    scene._camera.translation = np.array(
        [0, 4, -3.5], dtype=np.float32
    )  # Adjusted camera position
    scene._camera.rotation = np.array(
