...
class TransformNormalBenchmark:
    def __init__(self, width=1920, height=1080):
        ...
        for _ in range(100):
            ...
            transform_vertex_normal(
                normal, rotation, camera_rotation, False
            )  # Without camera
            transform_vertex_normal(
                normal, rotation, camera_rotation, True
            )  # With camera
        ...
        for _ in range(100):
            transform_vertex_normal(
                warm_up_normal, warm_up_rotation, warm_up_camera_rotation, False
            )
    ...
    def benchmark_transform(self, normal, rotation, camera_rotation, has_camera):
        ...
        for _ in range(NUM_LOOPS):
            ...
            total_times_jit.append(
                (end_time - start_time) * 1000
            )  # Convert to milliseconds

        return {"jit": np.mean(total_times_jit)}


def run_benchmark(benchmark, normal, rotation, camera_rotation, has_camera, name):
    """Run benchmark for normal transformation"""
    avg_times = benchmark.benchmark_transform(
        normal, rotation, camera_rotation, has_camera
    )
...
def main():
    ...
    run_benchmark(
        benchmark, normal, rotation, camera_rotation, False, "Rotation Only (No Camera)"
    )

    # Test with rotation and camera
    camera_rotation = np.array([np.pi / 6, np.pi / 6, np.pi / 6])
    run_benchmark(
        benchmark, normal, rotation, camera_rotation, True, "Rotation with Camera"
    )
